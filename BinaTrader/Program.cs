        ...
        private static void PrintOrderBook(string symbol, int n)
        {
            var OutputThread = new Thread(() =>
            {
                while (true)
                {
                    try
                    {
                        var bidsTop = new List<KeyValuePair<decimal, decimal>>();
                        var asksTop = new List<KeyValuePair<decimal, decimal>>();
                        ...
                        for (int i = 0; i < n; i++)
                        {
                            ...
                            var P2buyOrders = strategy.StrategyP2.P2_BuyOrders.Where(o => o.Price == bidPrice || o.Price == askPrice).ToList();
                            var P2sellOrders = strategy.StrategyP2.P2_SellOrders.Where(o => o.Price == bidPrice || o.Price == askPrice).ToList();
